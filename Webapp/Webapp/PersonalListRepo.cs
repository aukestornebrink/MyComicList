...
public class PersonalListRepo
{
    public List<PersonalLists> GetPersList(int order) //Fill list with SQL data
    {
        ...
        var sql =
            @"SELECT Name, Series, Summary, Photo_front, Author 
            FROM comics.comic_information ORDER BY (CASE WHEN @Order = 0 THEN Name WHEN @Order = 1 
                THEN Series WHEN @Order = 2 THEN Author END)"; //SQL Query
